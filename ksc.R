...
casos.sim <- rmultinom(1,casos_total, (casos_total*dados$Pop)/pop_total)

cluster.max <- data.frame()
llr.max <- data.frame()
for(i in 1:100){
  dados$Casos <- rmultinom(1, casos_total, (casos_total*dados$Pop)/pop_total)
  resultado <- llr.func(dados)
  llr.max <- rbind(llr.max,resultado[which.max(as.numeric(resultado$llr_z)),])
}
