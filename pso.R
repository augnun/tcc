...
vizinhos <- lapply(mat.adj, function(x) which(x == 1, arr.ind = T))
...
for(i in 1:nrow(dados)){
  ...
  ifelse(c_z > mu_z,
         llr_z <-
           c_z * log(c_z / mu_z) + (casos_total - c_z) * log((casos_total - c_z) /
                                                               (casos_total - mu_z)),
         llr_z <- 0)
  resultado <- rbind(resultado,cbind(zona, llr_z))
  for(j in 1:nrow(dados)){
    ...
    mu_z <- casos_total * (n_z / pop_total)
    if(c_z > mu_z){
           llr_z <- append(llr_z, c_z * log(c_z / mu_z) + (casos_total - c_z) * log((casos_total - c_z) /
                                                                 (casos_total - mu_z)))
           ...
           zonas_candidatas <- rbind(zonas_candidatas,I(zona, llr_z))
    }
